# h3-prompts/ — MiniMax-H3 prompt toolkit (ref2va / full-reference format)
...
## The ref2va / full-reference prompt format

Scene pools are text files; blocks separated by a `---` line. One block = one clip:

```
For the target video, at 0.00 seconds into the target video, <Picture N> (from Shot M) is fully referenced.   ← reference-mode header (ref2va); omit for plain T2VA
...
- The same prompt format is what the `blender-director` preset writes into its `-cues-h3.md` files (guide in that skill dir: `VIDEO_PROMPT_WRITING_GUIDE_ref_en.md`, source HF `MiniMaxAI/MiniMax-H3`)
